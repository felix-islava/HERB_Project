...
describe('Herb',()=>{
    ...
    it('Create MMIC User',()=>{
        ...
        cy.get('#spree_user_last_name').type(`MMIC${timestamp}`)

        cy.get('#birthdaypicker').type('01/01/1980')
        cy.get('#spree_user_email').type(`felix+mmic${timestamp}@herb.delivery`)
        ...
        cy.get('#spree_user_user_type_mmic').click()
        ...
        cy.get('#mmic > .dropzone-msg > .input-group > .custom-file > .custom-file-btn').attachFile('example3.jpg', { subjectType: 'drag-n-drop'});
        
        var MMICNumber = Math.floor(100000000 + Math.random() * 9000);
        console.log(MMICNumber);
        cy.get('#spree_user_mmic_number').type(MMICNumber)
        cy.get('#mmicexpirationdatepicker').type('12122026')
