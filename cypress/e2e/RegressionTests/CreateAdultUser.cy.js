...
describe('Herb',()=>{
    ...
    it('Create Adult User',()=>{
        cy.visit('/store')
        cy.get('#ageGateModal div:nth-child(1) > button').click({multiple: true}, {force: true})
        ...
        cy.get('#spree_user_last_name').type(`Adult${timestamp}`)

        cy.get('#birthdaypicker').type('01/01/1980')
        cy.get('#spree_user_email').type(`felix+adult${timestamp}@herb.delivery`)
